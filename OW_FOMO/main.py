...
import pandas as pd
from engine import viz, evaluate, train_one_epoch
...
def get_args_parser():
    ...
    ################ OWOD ################
    parser.add_argument('--unmatched_boxes', action='store_true')
    parser.add_argument('--nc_epoch', default=0, type=int)  # Epoch to start using nc (novel class objects) as GT 
...
def main(args):
    print(args)
    # Initialize distributed mode and print git info
    ...
    # Initialize parameters for negative supervision
    neg_sup_ep = [1, 10, 100]
    ...
    if (len(neg_sup_ep) > 1 or len(neg_sup_lr) > 1) and args.image_conditioned and args.att_refinement:
        for eps in tqdm(neg_sup_ep, desc='Epochs', leave=False):
            for lr in tqdm(neg_sup_lr, desc='lr', leave=False):
                ...
                #     print(f"Training in epoch {ep}...")
                #     train_one_epoch(model, data_loader_train, ep, args)
                train_one_epoch(model, data_loader_train, eps, args)
    ...
    '''
    # Set negative supervision parameters in args
    args.neg_sup_ep = epochs
    args.neg_sup_lr = 1e-5

    # Build model and postprocessors
    model, postprocessors = build_model(args)
    model.to(device)
    ...
    for epoch in range(epochs):
        model.train()
        train_one_epoch(model, data_loader_train, epoch, args)
        # save model weights
    ...
    # Evaluate with different unknown methods
