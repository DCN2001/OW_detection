...
class OWEvaluator(object):
    def __init__(self, voc_gt, args=None, use_07_metric=True, ovthresh=list(range(50, 100, 5))):
        ...
        self.unknown_class_index = args.PREV_INTRODUCED_CLS + args.CUR_INTRODUCED_CLS
    ...
    def update(self, predictions):
        for img_id, pred in predictions.items():
            ...
            print(f"len(clases): {len(classes)}")
            for (xmin, ymin, xmax, ymax), cls, score in zip(pred_boxes.tolist(), classes, pred_scores.tolist()):
                xmin += 1
                ymin += 1
                print(f"img_id: {img_id} class: {cls} score: {score}  xmin: {xmin}  ymin: {ymin}  xmax: {xmax}  ymax: {ymax}")
...
def voc_eval(detpath,
    ...
    imagenames = [f'{os.path.splitext(x)[0]}' for i, x in enumerate(lines)]
